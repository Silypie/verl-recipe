...
class AgentLoopWorkerForRewardQueue(AgentLoopWorker):
    ...
    def _build_reward_input(
        self, padded_output: _InternalAgentLoopOutput, kwargs: dict, validate: bool
    ) -> DataProto:
    ...
    async def _impl_generate_single_for_reward_queue(
        ...
        output = await self._run_reward_agent_loop(
            sampling_params, trajectory_info[0], **kwargs
        )
        ...
        default_extra_keys = {
            "turn_scores", "tool_rewards", "min_global_steps", "max_global_steps",
            "extras", "inference_start_timestamp", "inference_end_timestamp", "inference_duration",
        }
