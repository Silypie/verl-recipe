...
class AsyncAgentLoopManager(FullyAsyncAgentLoopManager):
    ...
    @auto_await
    async def generate_single_for_reward_queue(
...
ray_metadata = getattr(FullyAsyncRollouter, "__ray_metadata__")
...
class Rollouter(OriginalRollouter):
    ...
    async def _process_sample_with_reward_queue(self, rollout_sample: RolloutSample):
        ...
        for i in range(n):
            ...
            task = asyncio.create_task(
                self.async_rollout_manager.generate_single_for_reward_queue(sub_batch)
            )
        ...
        while pending:
            ...
            for task in done:
                ...
                try:
                    ...
                    print(
                        f"[FullyAsyncRollouter][Processor] Inference FAILED for {sample_id}[{i}]: {e}"
                    )
    ...
    async def _process_single_sub_reward_item(self, item: SubRewardDataItem):
        ...
        if not self.reward_loop_worker_handles:
            print(f"[FullyAsyncRollouter][RewardConsumer] No reward_loop_worker_handles, discard: {sample_id}[{item.sub_index}]")
            self.dropped_stale_samples += 1
            return
